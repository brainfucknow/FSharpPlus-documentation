...
| Find a practical pattern from a problem | `references/real-world-use-cases.md` |
| Optional actions, configuration, or mapping interoperability | `references/application-workflows.md` |
| Immutable repository state or nested record updates | `references/state-and-optics.md` |
| Accumulating diagnostics or custom domain containers | `references/validation-and-custom-types.md` |
| Codec composition or asynchronous optional routing | `references/codecs-and-composed-effects.md` |

## Non-negotiable rules

1. Always compile-verify F#+ code before presenting it; SRTP errors only appear at compile time.
2. Start with the narrowest import that owns the API: usually `open FSharpPlus`; add `FSharpPlus.Data`, `FSharpPlus.Lens`, or `FSharpPlus.Operators` only for their types, optics, or operator style.
